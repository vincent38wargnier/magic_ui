...
def tools_condition(state: GraphState):
    ...
    print("🔄 UI GENERATOR WORKFLOW TRANSITION")
    ...
    if not tool_calls:
        print(
            f"• No tools requested - Completing workflow ({recursion_count}/{recursion_limit})"
        )
        return "finish"

    if recursion_count > recursion_limit:
        print(
            f"• Recursion limit reached - Forcing completion ({recursion_count}/{recursion_limit})"
        )
        return "end"

    print(
        f"• Tools pending execution - Continuing workflow ({recursion_count}/{recursion_limit})"
    )
    return "continue"
